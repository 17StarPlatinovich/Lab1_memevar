            ...
            for (int j = 0; j < x.length; j++) {
                switch(c[i]){
                    case 4: {
                        ...
                        amogus = amogus/3;
                        amogus = amogus/4;
                        imposter = Math.pow(x[j], 2/x[j]);
                        amogus = Math.pow(amogus, imposter);
                        amogus = 0.75 + amogus;
                        Jerma985 = Math.sin(x[j]);
                        Jerma985 = Math.atan(Jerma985);
                        whentheImposterissus = Jerma985/amogus;
                        d[i][j] = Math.pow(whentheImposterissus, 2);
                        break;
                    }
                    case 6: case 7: case 8: case 9: case 14: case 15:
                            d[i][j] = Math.pow(Math.pow(pi * Math.pow((0.5 - x[j]), 3), Math.asin((x[j] + 1.5) / 13) / 2), 2);
                            break;
                    default:
